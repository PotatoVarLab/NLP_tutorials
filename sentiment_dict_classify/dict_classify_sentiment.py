...
class DictClassifier(object):
    ...
    def analyse_sentence(self, sentence, runout_filepath=None, print_show=True):
        ...
        if comment_analysis["score"] > 0:
            print('\ncomment_analysis["score"]:',comment_analysis["score"])
            return 1
...
if __name__ == '__main__':
    dictcls = DictClassifier()

    sent = '这真是一个好消息,6.18狂欢,快，态度好！味道不错'
    sent = '这真是一个坏消息,6.18狂欢,又慢，态度又不好！味道差'

    # sent = '天朝的zf，tmd就是吸老百姓的血，税又多，又重'

    res = dictcls.classify(sent)
    print('Finally result: {}'.format(res))
