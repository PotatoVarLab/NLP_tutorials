...
## Required parameters
flags.DEFINE_string(
        "data_dir", '/home/ypl/huizi/bert/data',
        "The input data dir. Should contain the .tsv files (or other data files) "
        "for the task.")

flags.DEFINE_string(
        "bert_config_file", '/home/ypl/huizi/bert/chinese_L-12_H-768_A-12/bert_config.json',
        "The config json file corresponding to the pre-trained BERT model. "
        "This specifies the model architecture.")

flags.DEFINE_string("task_name", 'chi', "The name of the task to train.")

flags.DEFINE_string("vocab_file", '/home/ypl/huizi/bert/chinese_L-12_H-768_A-12/vocab.txt',
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
        "output_dir", '/home/ypl/huizi/bert/data/database/output',
        "The output directory where the model checkpoints will be written.")

## Other parameters

flags.DEFINE_string(
        "init_checkpoint", '/home/ypl/huizi/bert/output',
...
class PredictHandler():
    ...
    def predicts(self, text_data):
        if FLAGS.do_predict:
            ...
            predict_file = os.path.join(FLAGS.output_dir, "predict.tf_record")
            tf_example = file_based_convert_examples_to_features(predict_examples, self.label_list,
            ...
            result = self.estimator.predict(input_fn=predict_input_fn)
            print(' \nPredict result:{}'.format(result))
            res_dic = {}
            num_written_lines = 0
            tf.logging.info("***** Predict results *****")
            for (j, prediction) in enumerate(result):
                probabilities = prediction["probabilities"]
                if j >= num_actual_predict_examples:
                    break
                num_written_lines += 1
                dicts = {}
                for i in range(len(probabilities)):
                    dicts[self.label_list[i]] = probabilities[i]
                print(dicts)
                dicts = sorted(dicts.items(), key=lambda x: x[1], reverse=True)
